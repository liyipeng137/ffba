...
def select_loma_pairs(
    ...
    *,
    ...
    stats = {
        ...
        "diversity": (
            "prefer neighbors farther than temporal_window from earlier choices; soft fallback"
            if temporal_window
            else "disabled"
        ),
...
class LoMaBackend:
    ...
    def __init__(self, device, feature_cache="cpu"):
        ...
        source = Path(__file__).resolve().parents[2] / "third_party" / "LoMa" / "src"
    ...
    def extract_batched(self, paths, batch_size, workers):
        from ffba.matching.loma_execution import (
            bucket_batches,
            image_shape,
            prepare_image,
            prefetch_batches,
...
def _run_loma_prior(
    ...
    import pycolmap
    from ffba.matching.loma_execution import GeometryQueue, bucket_batches
