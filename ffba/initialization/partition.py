...
from ffba.initialization.shortest_path import most_similar_path
...
class Sequence(ABC):
    ...
    def change_reference_frame(self, reference_frame_idx=None):
        ...
        homogenous_extris = convert_to_homogeneous_matrix(
            self.merged_predictions["extrinsic"]
        )

        reference_frame_extrinsic_inv = torch.linalg.inv(
            homogenous_extris[reference_frame_idx]
        )
        new_reference_extris = homogenous_extris @ reference_frame_extrinsic_inv

        self.merged_predictions["extrinsic"] = remove_homogeneous_row(
            new_reference_extris
        )
    ...
    def similarity_transform(extri, transform, scale):
        ...
        converted_curr_t = (
            scale * extri[:, :3, 3:] - converted_curr_R @ transform[None, :3, 3:]
        )  # N, 3, 1
    ...
    def transform_to_shared_frame(self, transforms, scales, node_id=0, parent_id=None):
        ...
        def from_numpy_alias(x):
            return torch.from_numpy(x).float().to(device="cpu")
        ...
        overlap_indices_tensor = torch.tensor(
            overlap_indices, dtype=torch.int, device="cpu"
        )
        ...
        for key in curr_node.keys():
            if not isinstance(curr_node[key], np.ndarray) and not isinstance(
                curr_node[key], torch.Tensor
            ):
                continue
            transformed_node[key] = from_numpy_alias(curr_node[key])[
                non_overlap_indices
            ]
        # Collect image ids to make image_name reordering easier later
        transformed_node["image_ids"] = (
            self.subset_to_img_ids[node_id][non_overlap_indices]
        ).to(device="cpu")
        ...
        if not is_leaf:
            ...
            for child_id in edges[node_id]:
                converted_child = self.transform_to_shared_frame(
                    transforms, scales, node_id=child_id, parent_id=node_id
                )
                for key in converted_child.keys():
                    if key not in ["tracks", "vis_scores", "points", "tracker_conf"]:
                        ...
                        transformed_node[key] = torch.cat(
                            [transformed_node[key], converted_child[key]], dim=0
                        )
    ...
    def split(self):
        ...
        if self.splitting_type == "interleave":
            ...
            for img in tail:
                ...
                for i in range(len(new_path) - 1):
                    sim_score = (
                        self.sim_matrix[new_path[i]][img]
                        + self.sim_matrix[new_path[i + 1]][img]
                    ) / 2
            ...
            while torch.sum(picked) != len(self.path):
                ...
                for i in range(old_start, len(self.path)):
                    ...
                    if (
                        (not picked[i])
                        and (
                            self.sim_matrix[self.path[start]][self.path[i]]
                            <= threshold1
                        )
                        and (
                            self.sim_matrix[self.path[start]][self.path[i]]
        ...
        if num_subsets >= 2:
            ...
            if last_subset_size < threshold:
                ...
                image_split[-2] = torch.stack(
                    [self.images[i] for i in first_subset_indices]
                )
                image_split[-1] = torch.stack(
                    [self.images[i] for i in second_subset_indices]
                )
                ...
                subset_to_img_ids[num_subsets - 2] = torch.tensor(
                    first_subset_indices, dtype=torch.int, device=self.device
                )
                subset_to_img_ids[num_subsets - 1] = torch.tensor(
                    second_subset_indices, dtype=torch.int, device=self.device
                )
...
def create_sequence(
    ...
    if sequence_type != "shortest_path":
        raise ValueError("The formal pipeline uses shortest_path partitioning")
    return ShortestPath(
        image_list,
        image_names,
        subset_size,
