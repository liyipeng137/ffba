...
def interleaved_multi_path_torch(
    ...
    if start_indices is not None:
        if dedup_starts:
            ...
            if len(starts) < n_paths:
                ...
                if remaining:
                    rand_perm = torch.tensor(remaining)[
    ...
    visited_sets = [
        torch.zeros(N, dtype=torch.bool, device=M.device) for _ in range(len(starts))
    ]
...
@torch.no_grad()
def two_opt_improve_torch(
    M: torch.Tensor, path: List[int], max_passes: int = 3
) -> List[int]:
...
def most_similar_path_old(M, n_starts=1, polish_top_k=1):
    path, weight, all_paths = interleaved_multi_path_torch(
        M, n_paths=n_starts, polish_top_k=polish_top_k
    )
    return path, weight
...
def farthest_neighbor_path_greedy(
    M: torch.Tensor, start: Optional[int] = None
) -> List[int]:
...
def regret_insertion_longest_path(
    ...
    while remaining.numel() > 0:
        ...
        if m > 1:
            ...
            gains_pos = torch.vstack(
                [gains_left.unsqueeze(0), gains_between, gains_right.unsqueeze(0)]
            )
        else:
            # degenerate, shouldn't happen since m>=2
            gains_pos = torch.vstack(
                [gains_left.unsqueeze(0), gains_right.unsqueeze(0)]
            )
        ...
        torch.argmax(torch.stack([regrets, top_vals[0]], dim=0), dim=1)[0]
        ...
        remaining = torch.tensor(
            [i for i in remaining.tolist() if i != node], dtype=torch.long
        )
    ...
    def run_once(start: int) -> List[int]:
        ...
        for _ in range(n - 1):
            ...
            for path, last, w, visited in beams:
                ...
                for j in cands:
                    expanded.append(
                        (path + [j], j, w + float(M[last, j].item()), visited | {j})
                    )
    ...
    def regret_insert_sequence(path: List[int], nodes: List[int]) -> List[int]:
        ...
        while remaining.numel() > 0:
            ...
            top_vals, top_idx = torch.topk(
                gains_pos, k=min(2, gains_pos.shape[0]), dim=0
            )
            if top_vals.shape[0] == 1:
                regrets = top_vals[0]
                best_pos = top_idx[0]
