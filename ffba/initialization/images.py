...
def load_image_tensors_from_dir(
    ...
    if len(shapes) != 1:
        raise ValueError(
            f"Pipeline images must have a common shape. Got shapes: {sorted(shapes)}"
        )
...
def center_crop_to_multiple(width, height, multiple):
    ...
    if new_width <= 0 or new_height <= 0:
        raise ValueError(
            f"Image too small to crop to a multiple of {multiple}: {width}x{height}"
        )
...
def scale_intrinsics_low_to_high(
    ...
    if intrinsic_np.ndim != 3 or intrinsic_np.shape[-2:] != (3, 3):
        raise ValueError(
            f"Expected intrinsics shape (3, 3) or (N, 3, 3), got {intrinsic_np.shape}"
        )
