...
from ffba.initialization.images import SUPPORTED_IMAGE_EXTS
...
def convert_nerfstudio_c2w_to_opencv_w2c(transform_matrix):
    ...
    if not np.allclose(c2w_nerfstudio[3], [0.0, 0.0, 0.0, 1.0], atol=1e-6):
        raise ValueError("Nerfstudio transform_matrix must end with [0, 0, 0, 1]")
    ...
    *,
    ...
    if not transforms_path.is_file():
        raise FileNotFoundError(
            f"Nerfstudio transforms file not found: {transforms_path}"
        )
    ...
    if not isinstance(frames, list) or not frames:
        raise ValueError(
            "Nerfstudio transforms.json must contain a non-empty frames list"
        )
    ...
    camera_centers = np.stack(
        [-pose[:3, :3].T @ pose[:3, 3] for pose in extrinsic.astype(np.float64)]
    )
