...
namespace sibr { 
	...
	public:
		 ...
		 */
		GaussianView(const sibr::BasicIBRScene::Ptr& ibrScene, uint render_w, uint render_h, const char* file, bool* message_read, int sh_degree, int sg_degree, bool white_bg = false, bool useInterop = true, int device = 0);
	...
	protected:
		...
		size_t allocdGeom = 0, allocdBinning = 0, allocdImg = 0, allocdTile = 0;
		void* geomPtr = nullptr, * binningPtr = nullptr, * imgPtr = nullptr, * tilePtr = nullptr;
		std::function<char* (size_t N)> geomBufferFunc, binningBufferFunc, imgBufferFunc, tileBufferFunc;
