...
namespace sibr { 
	...
	GaussianData::GaussianData(int num_gaussians, float* mean_data, float* rot_data, float* scale_data, float* alpha_data, float* color_data, int sh_degree)
	{
		...
		glNamedBufferStorage(colorBuffer, num_gaussians * sizeof(float) * (sh_degree + 1) * (sh_degree + 1) * 3, color_data, 0);
	}
	...
	GaussianSurfaceRenderer::GaussianSurfaceRenderer( void )
	{
		...
		_paramColorStride.init(_shader, "color_stride");
	...
	int	GaussianSurfaceRenderer::process(int G, const GaussianData& mesh, const Camera& eye, IRenderTarget& target, float limit, int stride, sibr::Mesh::RenderMode mode, bool backFaceCulling)
	{
		glBindFramebuffer(GL_FRAMEBUFFER, fbo);
