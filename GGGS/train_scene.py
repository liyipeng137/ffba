...
class StageConfig:
    ...
    enable_depth_prior: bool        # 是否启用深度先验损失
...
def training(
    ...
    has_depth_dir = os.path.exists(os.path.join(dataset.source_path, dataset.depth_prior_dir))
    has_confidence_dir = os.path.exists(os.path.join(dataset.source_path, dataset.depth_confidence_dir))
    ...
    has_loaded_depth_prior = any(cam.depth_prior is not None for cam in full_res_train_cameras)
    has_loaded_confidence_prior = any(cam.depth_confidence is not None for cam in full_res_train_cameras)
    ...
    if has_confidence_dir and not has_loaded_confidence_prior:
        print("[Pipeline][Warn] confidence directory exists but no valid confidence priors were loaded.")
    ...
    print(f"[Pipeline] has_depth_dir={has_depth_dir} (loaded_depth_priors={has_loaded_depth_prior})")
    print(f"[Pipeline] has_confidence_dir={has_confidence_dir} (loaded_confidence_priors={has_loaded_confidence_prior})")
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        loss = (
            ...
            + lambda_depth_prior_cur * depth_prior_loss
        )
        loss.backward()
