...
def so3_relative_angle(rot_gt, rot_pred, eps=1e-4):
    """
    Compute the relative angle between two rotation matrices.

    Args:
        rot_gt (torch.Tensor): Ground truth rotation matrix (B, 3, 3).
        rot_pred (torch.Tensor): Predicted rotation matrix (B, 3, 3).
        eps (float): Small value to avoid division by zero.

    Returns:
        torch.Tensor: Relative angle in radians.
    """
    # Compute the cosine of the angle
    rot_gt, rot_pred = torch.from_numpy(rot_gt), torch.from_numpy(rot_pred)
    cos_angle = (torch.einsum("...ii",rot_gt.transpose(1, 2).bmm(rot_pred)) - 1) / 2
    #clip
    cos_angle = cos_angle.clip(-1,1)
    return torch.acos(cos_angle).numpy()
...
def compare_translation_by_angle(t_gt, t, eps=1e-15, default_err=1e6):
    ...
    t_gt, t = torch.from_numpy(t_gt), torch.from_numpy(t)
