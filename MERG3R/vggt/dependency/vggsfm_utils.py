...
def farthest_point_sampling(D: torch.Tensor, num_samples: int, start_index: int = 0):
    """
    Farthest Point Sampling on a pairwise distance matrix.

    Args:
        D: (N, N) symmetric, non-negative, zero diag (distances between frames)
        num_samples: number of indices to select (will cap at N)
        start_index: seed index to start from

    Returns:
        List[int]: selected indices (length = min(num_samples, N))
    """
    assert D.ndim == 2 and D.shape[0] == D.shape[1], "D must be square (N,N)"
    N = D.shape[0]
    k = min(num_samples, N)

    # Work in float32 and ensure no negative distances
    D = D.to(dtype=torch.float32).clamp_min_(0)

    selected = [int(start_index)]
    # current best distance to the selected set (min across rows in 'selected')
    # initialize with distances to the seed
    best = D[start_index].clone()  # (N,)

    # never select already selected points
    best[selected] = -1.0  # sentinel below any valid distance

    for _ in range(1, k):
        # pick the farthest from the selected set: max of current best dists
        idx = int(torch.argmax(best).item())
        selected.append(idx)

        # update the min-distance-to-set using the newly selected row
        best = torch.minimum(best, D[idx])
