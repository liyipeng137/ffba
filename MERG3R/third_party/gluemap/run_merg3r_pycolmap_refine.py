...
def parse_args():
    ...
    parser.add_argument(
        ...
        default=False,
        help=(
            "Save the full virtual predictions_dict to "
            "virtual_tracks_debug.pt."
        ),
    )
...
def normalize_intrinsics(intrinsic, num_images):
    intrinsic = np.asarray(intrinsic, dtype=np.float64)
    if intrinsic.ndim == 4 and intrinsic.shape[0] == 1:
        intrinsic = intrinsic[0]
    if intrinsic.ndim == 2 and intrinsic.shape == (3, 3):
        intrinsic = np.repeat(intrinsic[None], num_images, axis=0)
    if intrinsic.shape != (num_images, 3, 3):
        raise ValueError(
            "Expected intrinsic shape (N,3,3), (1,N,3,3), or (3,3), got "
            + f"{intrinsic.shape} for {num_images} images"
        )
    return intrinsic


def normalize_depth_array(array, num_images, name):
...
def load_raw_geometry(artifact_dir, num_images):
    path = artifact_dir / "raw_geometry.npz"
    if not path.exists():
        raise FileNotFoundError(
            f"{path} is required for virtual-track diagnostics. "
            "Re-run MERG3R/export_merg3r_refine_inputs.py to export raw "
            "depth/depth_conf artifacts, or pass --no-build_virtual_tracks."
        )
    data = np.load(path)
    depth = normalize_depth_array(data["depth"], num_images, "depth")
    depth_conf = None
    if "depth_conf" in data:
        depth_conf = normalize_depth_array(
            data["depth_conf"], num_images, "depth_conf"
        )
    return depth, depth_conf


def to_homogeneous_w2c(extrinsic):
    extrinsic = normalize_extrinsic(extrinsic)
    result = np.repeat(
        np.eye(4, dtype=np.float64)[None], len(extrinsic), axis=0
    )
...
def average_intrinsics_with_gluemap(initial_intrinsics, camera_model):
    ...
    intrinsics_all = [
        torch.from_numpy(initial_intrinsics)
        .to(torch.float64)
        .unsqueeze(0)
    ]
...
def save_intrinsics_artifacts(
    ...
    np.savez_compressed(
        ...
        shared_global_intrinsic=averaged_intrinsics[0].astype(
...
def summarize_virtual_pair_coverage(predictions_dict):
    ...
    return {
        ...
        "zero_pairs": int(np.sum(np.asarray(counts, dtype=np.int64) == 0))
        if counts
        else 0,
...
def build_virtual_track_diagnostics(
    ...
    skipped_centers = [
        idx for idx in range(num_images) if idx not in center_set
    ]
    ...
    for group in groups:
        ...
        if depth_conf is not None:
            predictions["depth_conf"] = torch.from_numpy(
                depth_conf[group_np]
        ...
        pose_scores, reprojection_valid_mask = (
            extractor._verify_by_reprojection_n2(
                depth_transformed,
                predictions["extrinsics"],
                predictions["intrinsics"],
            )
        ...
        predictions_dict["extrinsics"].append(
            predictions["extrinsics"].detach().cpu()
        )
        predictions_dict["intrinsics"].append(
            predictions["intrinsics"].detach().cpu()
        )
        predictions_dict["pose_scores"].append(pose_scores.detach().cpu())
        predictions_dict["tracks_virtual"].append(
            tracks_virtual.detach().cpu()
        )
        predictions_dict["points3d_virtual"].append(
            points3d_virtual.detach().cpu()
        )
        predictions_dict["valid_virtual"].append(valid_virtual.detach().cpu())
        predictions_dict["isnegative_virtual"].append(
            isnegative_virtual.detach().cpu()
        )
...
def main():
    ...
    initial_intrinsics_all = normalize_intrinsics(
        poses["intrinsic"], extrinsic.shape[0]
    )
    ...
    (
        ...
        global_intrinsics,
        intrinsics_mapping,
    ) = average_intrinsics_with_gluemap(initial_intrinsics, camera_model)
    stats["timing"]["intrinsics_averaging"] = time.time() - t0
    intrinsic = averaged_intrinsics[0]
    intrinsics_stats = summarize_intrinsics(
    ...
    if args.build_virtual_tracks:
        ...
        t0 = time.time()
        raw_depth_all, raw_depth_conf_all = load_raw_geometry(
            artifact_dir, len(initial_intrinsics_all)
        )
        depth = raw_depth_all[kept_indices]
        depth_conf = (
            raw_depth_conf_all[kept_indices]
