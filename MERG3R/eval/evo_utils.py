...
# def make_traj_nonstep(args) -> PoseTrajectory3D:
...
def eval_metrics(pred_traj, gt_traj=None, seq="", filename="", sample_stride=1):
    ...
    ate_median = np.median(ate_result.np_arrays["error_array"])
    ate_mean = np.mean(ate_result.np_arrays["error_array"])
    ...
    for delta in delta_list:
        ...
        rot_median = np.median(rpe_rots_result.np_arrays["error_array"])
        rot_mean = np.mean(rpe_rots_result.np_arrays["error_array"])
        rpe_rots.append(rot)
        # except:
        #     print("RPE rotation failed")
        #     rot_median = -1
    ...
    for delta in delta_list:
        ...
        trans_median = np.median(rpe_transs_result.np_arrays["error_array"])
        trans_mean = np.mean(rpe_transs_result.np_arrays["error_array"])
        rpe_transs.append(trans)

    rpe_trans, rpe_rot = np.mean(rpe_transs), np.mean(rpe_rots)
    with open(filename, "w+") as f:
        f.write(f"APE w.r.t. translation part (m) - mean:            {ate_mean:.3f}\n")
        f.write(f"RPE w.r.t. rotation angle in degrees (deg) - mean: {rot_mean:.3f}\n")
        f.write(f"RPE w.r.t. translation part (m) - mean:            {trans_mean:.3f}\n\n")

        f.write(f"APE w.r.t. translation part (m) - median:            {ate_median:.3f}\n")
        f.write(f"RPE w.r.t. rotation angle in degrees (deg) - median: {rot_median:.3f}\n")
        f.write(f"RPE w.r.t. translation part (m) - median:            {trans_median:.3f}\n\n")

        f.write(f"Seq: {seq} \n\n")
        f.write(f"{ate_result}")
        if rpe_rots_result is not None:
